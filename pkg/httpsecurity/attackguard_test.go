...
func TestAttackGuardBlocksBurstAndExpires(t *testing.T) {
	...
	var blocked bool
	for i := 0; i < 600; i++ {
		block, blocked = guard.ObserveFast("203.0.113.7", "/same", false, now.Add(time.Duration(i)*time.Millisecond))
	}
	if !blocked || block.Reason != "dos" {
		t.Fatalf("expected burst block, got blocked=%v reason=%q", blocked, block.Reason)
	}
	if _, ok := guard.Check("203.0.113.7", block.ExpiresAt.Add(time.Second)); ok {
